<form id="rekening6__edit" method="POST" action="{{ route('rekening6.update', $item->id) }}">
    @csrf
    @method('PUT')
    <div class="form-group mb-2">
        ...
        <select required name="rekening1_id" class="form-control">
            <option value="{{ $item->rekening5->rekening4->rekening3->rekening2->rekening1_id }}" selected>[{{
                $item->rekening5->rekening4->rekening3->rekening2->rekening1->kode }}] {{
                $item->rekening5->rekening4->rekening3->rekening2->rekening1->nama
                }}</option>
        </select>
    ...
    <div class="form-group mb-2">
        ...
        <select required name="rekening2_id" class="form-control">
            <option value="{{ $item->rekening5->rekening4->rekening3->rekening2_id }}" selected>[{{
                $item->rekening5->rekening4->rekening3->rekening2->kode }}] {{
                $item->rekening5->rekening4->rekening3->rekening2->nama
                }}</option>
        </select>
    ...
    <div class="form-group mb-2">
        ...
        <select required name="rekening3_id" class="form-control">
            <option value="{{ $item->rekening5->rekening4->rekening3_id }}" selected>[{{
                $item->rekening5->rekening4->rekening3->kode }}] {{
                $item->rekening5->rekening4->rekening3->nama
                }}</option>
        </select>
    ...
    <div class="form-group mb-2">
        ...
        <select required name="rekening4_id" class="form-control">
            <option value="{{ $item->rekening5->rekening4_id }}" selected>[{{ $item->rekening5->rekening4->kode }}] {{
                $item->rekening5->rekening4->nama
                }}</option>
        </select>
        <input type="hidden" required name="kode4" class="form-control" value="{{ $item->kode4 }}">
    </div>
    <div class="form-group mb-2">
        <label class="form-control-label mb-0">Rek. Rincian Objek <span class="text-danger">*</span></label>
        <select required name="rekening5_id" class="form-control">
            <option value="{{ $item->rekening5_id }}" selected>[{{ $item->rekening5->kode }}] {{
                $item->rekening5->nama
                }}</option>
        </select>
        <input type="hidden" required name="kode5" class="form-control" value="{{ $item->kode5 }}">
    </div>
    <div class="form-group mb-2">
        ...
        <div class="input-group input-group-merge">
            <div class="input-group-prepend">
                <span id="prefix_kode" class="input-group-text pr-1">{{ $item->kode1 }}.{{ $item->kode2 }}.{{
                    $item->kode3 }}.{{
                    str_pad($item->kode4, 2, '0', STR_PAD_LEFT) }}.{{
                    str_pad($item->kode5, 2, '0', STR_PAD_LEFT) }}.</span>
            </div>
            <input type="number" required name="kode6" class="form-control" value="{{ $item->kode6 }}">
        </div>
    </div>
...
<script>
    ...
    $("[name='rekening1_id']").on('select2:select', function (e) {
        ...
        // rekening 5
        $("[name='kode5']").val(null);
        $("[name='rekening5_id']").val(null);
        $("[name='rekening5_id']").trigger('change.select2');
        $('#prefix_kode').html(null);
    });
</script>
<script>
...
<script>
    $("[name='rekening5_id']").select2({
        placeholder: "Pilih Rekening Induk...",
        // allowClear: true,
        escapeMarkup: function (m) { return m; },
        ajax: {
            url: "{{ route('rekening5.select') }}",
            type: "GET",
            dataType: 'json',
            delay: 250,
            data: function (params) {
                return {
                    search: params.term,
                    rekening4_id: $("[name='rekening4_id']").val()
                };
            },
            processResults: function (response) {
                removeLoader('sm');
    ...
    $("[name='rekening5_id']").on('select2:select', function (e) {
        console.log(e);
        $('[name="kode5"]').val(`${e.params.data.item.kode5}`);
        $('#prefix_kode').html(`${$('[name="kode1"]').val()}.${$('[name="kode2"]').val()}.${$('[name="kode3"]').val()}.${Number(e.params.data.item.kode4).toString().padStart(2, "0")}.${Number(e.params.data.item.kode5).toString().padStart(2, "0")}.`);
    });
</script>
<script>
    $("#rekening6__edit").on("submit", function (event) {
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                $('#rekening6').DataTable().ajax.reload(null, false);
                $(".modal").modal("hide");
            },
            error: function (xhr) {
