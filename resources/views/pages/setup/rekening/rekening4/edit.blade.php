<form id="rekening4__edit" method="POST" action="{{ route('rekening4.update', $item->id) }}">
    @csrf
    @method('PUT')
    <div class="form-group mb-2">
        ...
        <select required name="rekening1_id" class="form-control">
            <option value="{{ $item->rekening3->rekening2->rekening1_id }}" selected>[{{
                $item->rekening3->rekening2->rekening1->kode }}] {{
                $item->rekening3->rekening2->rekening1->nama
                }}</option>
        </select>
    ...
    <div class="form-group mb-2">
        ...
        <select required name="rekening2_id" class="form-control">
            <option value="{{ $item->rekening3->rekening2_id }}" selected>[{{ $item->rekening3->rekening2->kode }}] {{
                $item->rekening3->rekening2->nama
                }}</option>
        </select>
    ...
    <div class="form-group mb-2">
        ...
        <select required name="rekening3_id" class="form-control">
            <option value="{{ $item->rekening3_id }}" selected>[{{ $item->rekening3->kode }}] {{
                $item->rekening3->nama
                }}</option>
        </select>
    ...
    <div class="form-group mb-2">
        ...
        <div class="input-group input-group-merge">
            ...
            <input type="number" required name="kode4" class="form-control" value="{{ $item->kode4 }}">
        </div>
    </div>
...
<script>
    ...
    $("[name='rekening3_id']").on('select2:select', function (e) {
        ...
        $('#prefix_kode').html(`${$('[name="kode1"]').val()}.${$('[name="kode2"]').val()}.${e.params.data.item.kode3}.`);
    });
</script>
<script>
    $("#rekening4__edit").on("submit", function (event) {
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                $('#rekening4').DataTable().ajax.reload(null, false);
                $(".modal").modal("hide");
            },
            error: function (xhr) {
