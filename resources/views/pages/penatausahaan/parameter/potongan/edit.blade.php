<form id="potongan__edit" method="POST" action="{{ route('potongan.update', $item->id) }}">
    @csrf
    @method('PUT')
    <div class="form-group mb-3">
        <label class="form-control-label mb-0">Jenis Potongan <span class="text-danger">*</span></label>
        <select required name="jenis_potongan_id" class="form-control">
            <option value="{{ $item->jenis_potongan_id }}">{{ $item->jenis_potongan->nama }}</option>
        </select>
    </div>
    <div class="form-group mb-3">
    ...
    <div class="form-group mb-3">
        ...
        <input name="kode_map" class="form-control" value="{{ $item->kode_map }}">
    </div>
    <div class="form-group mb-1">
        <label class="form-control-label mb-0">Tarif (%)</label>
        <input type="number" step="0.01" name="tarif" class="form-control" value="{{ $item->tarif }}">
    </div>
    <div class="form-group mb-3">
        <label class="form-control-label mb-0">Rumus Perhitungan</label>
        <textarea name="perhitungan" class="form-control">{{ $item->perhitungan }}</textarea>
    </div>
    <div class="form-group mb-1">
        ...
        <select required name="is_dpp_harga_jual" class="form-control">
            <option value="0" @selected($item->is_dpp_harga_jual == 0)>Tidak</option>
            <option value="1" @selected($item->is_dpp_harga_jual == 1)>Iya</option>
        </select>
    </div>
...
<script>
    $("#potongan__edit").on("submit", function (event) {
